...
describe('AI Tools', () => {
  test('listBills filters by status', async () => {
    const result: any = await (aiTools.listBills as any).execute({ status: ['Draft'] }, { toolCallId: '1', messages: [] });
  ...
  test('getApprovals returns only Pending Approval', async () => {
    const result: any = await (aiTools.getApprovals as any).execute({}, { toolCallId: '2', messages: [] });
    expect(result.count).toBeGreaterThan(0);
    result.bills.forEach((b: any) => expect(b.status).toBe('Pending Approval'));
  });

  test('getCashflow returns valid numbers', async () => {
    const result: any = await (aiTools.getCashflow as any).execute({}, { toolCallId: '3', messages: [] });
  ...
  test('sumByCategory returns correct sum', async () => {
    const result: any = await (aiTools.sumByCategory as any).execute({ category: 'Software' }, { toolCallId: '4', messages: [] });
